    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm text-slate-500">{completed} of {candidates.length} complete</span>
          <button type="button" className="btn-primary" onClick={save}>Save benchmarks</button>
          {status && <span className="text-sm text-slate-500" role="status">{status}</span>}
        </div>
      </div>
        ...
        {candidates.map((candidate) => {
          ...
          return (
            <article key={candidate.watch.id} className="card grid gap-4 p-4 md:grid-cols-[120px_1fr_260px]">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">{candidate.watch.brand}</p>
                <h2 className="font-semibold text-slate-900">{candidate.watch.model}</h2>
