...
**Overlap grouping uses ratios, not bands.** `src/lib/overlap.ts` groups entries that would be the same purchase: same category, `SIZE_TOLERANCE_MM` (1.5) apart on diameter, within `PRICE_RATIO_LIMIT` (1.5x) on the all-in USD price. It deliberately does *not* reuse the scoring price bands — a $980 and a $1050 diver fall either side of a band edge and are obviously the same decision. Groups are cliques, not chains: every member is compatible with every other, so a 39.4mm middle can't merge a 38mm and a 41mm. Value and design leaders are reported separately and never combined; a watch missing a category, diameter, or price is returned in `unchecked` with a reason rather than grouped on a guess.
